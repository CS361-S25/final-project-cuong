...
EMP_BUILD_CONFIG(MyConfigType,
    ...
    VALUE(START_NUM, int, 100, "How many organisms should the world be populated with at first?"),
    VALUE(UPDATE_NUM, int, 1001, "How many updates should be done for the world state?"),
    VALUE(UPDATE_RECORD_FREQUENCY, int, 50, "How many updates should be done between each datafile record entry?"),
    VALUE(WORLD_LEN, int, 30, "How long is the world?"),
    VALUE(WORLD_WIDTH, int, 30, "How wide is the world?"),
    VALUE(CELL_SIZE, int, 10, "How large is each cell in the world?"),
    VALUE(MUTATION_RATE, float, 0.05, "How likely wil each genome bit will be mutated?"),
